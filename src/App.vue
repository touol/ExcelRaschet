...
<template>
  ...
  >
    ...
    <template v-if="DocLink.doc_type_id==4">
      <Spec v-model="SpecItem"/>
    </template>
    <template #footer>
      <Button
        label="Отмена"
...
<script setup>
    ...
    DocOrderLink:{
      ...
      update:{
        ...
        click: async (data, columns,table,filters) => {
          ...
          if(DocLink.value.doc_type_id == 4){
            try {
              SpecItem.value = await api_SpecItem.get(DocLink.value.doc_id)
            } catch (error) {
              notify('error', { detail: error.message });
              return
            }
  ...
  const fValidateDocLink = async (step) => {
    ...
    if(step == 1){
      console.log('DocLink.value.doc_type_id',DocLink.value.doc_type_id)
      switch(DocLink.value.doc_type_id){
        case '4':
          try {
            const response = await api_SpecItem.action('specification/get_default_value',{raschet_id:DocLink.value.order_id})
            SpecItem.value = response.data.out
  ...
  const saveItem = async () => {
    ...
    switch(DocLink.value.doc_type_id){
      ...
      case '4':
        ...
        if (SpecItem.value.id) {
          try {
            await api_SpecItem.update(SpecItem.value)
            createDocDialog.value = false;
            updateDocDialog.value = false
            SpecItem.value = {};
          } catch (error) {
          ...
          try {
            const response = await api_SpecItem.create(SpecItem.value)
            // console.log('response',response)
            DocLink.value.doc_id = response.data.id
